...
class ExchangeRateJobService {
   ...
   * Runs every 4 hours for better reliability
  ...
  startJob(): void {
    ...
    // Updated: Every 4 hours for better reliability
   ...
   */
  private async getRecentRateId(): Promise<number | null> {
    try {
      ...
      return recentRate?.id || null;
    } catch (error) {
      logger.error('Failed to get recent rate ID', { error });
      return null;
    }
  }
